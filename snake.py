...
def main():
    ...
    # 计算游戏区域的宽度和高度（以格子为单位）
    area_width = Area_x[1] - Area_x[0] + 1
    area_height = Area_y[1] - Area_y[0] + 1
        ...
        if not game_over:
            ...
            if curTime-last_move_time>speed: ###
                if not pause:
                    ...
                    # 边界穿越处理
                    # 如果超出左右边界，从另一边出现
                    if next_s[0] < Area_x[0]:
                        next_s = (Area_x[1], next_s[1])
                    elif next_s[0] > Area_x[1]:
                        next_s = (Area_x[0], next_s[1])
                    
                    # 如果超出上下边界，从另一边出现
                    if next_s[1] < Area_y[0]:
                        next_s = (next_s[0], Area_y[1])
                    elif next_s[1] > Area_y[1]:
                        next_s = (next_s[0], Area_y[0])
